...
@pytest.fixture
def tes_file(filename, mode, crosscheck=False):
    if mode:
        s = "memory"
    else:
        s = "disk"
...
def test_file_all_modes(filename):
    ...
    tes_file(filename, False, False)
    tes_file(filename, True, False)
    tes_file(filename, False, True)
    tes_file(filename, True, True)
